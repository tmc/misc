/*
Command clean-cc-git-history removes AI-generated and co-authorship attribution
lines from Git commit messages and preserves original commit information via
Git notes.

This tool processes Git commits on the current branch (by default, only unpushed
commits) and removes lines containing "Generated with" and "Co-Authored-By" from
commit messages. It preserves the original commit SHA and modification history
through Git notes for auditability.

USAGE

	clean-cc-git-history [flags]

The tool operates on the current Git repository and branch. By default, it only
processes commits that haven't been pushed to the remote repository.
...
with team members before running on shared branches. Never run on commits that
have already been pushed to shared repositories unless you coordinate with all
collaborators.
...
The tool will exit with an error if run outside a Git repository or if Git
operations fail.
*/
package main
