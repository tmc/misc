#!/usr/bin/env bash
# ctx-src.sh
# Process source code files for LLM input, respecting .gitignore and custom exclusions.
# Usage: ctx-src [options] [directory] [<pathspec>...] [-- <pathspec>...]
#
# For full usage instructions, run: ctx-src --help
set -euo pipefail
...
Process source code files in a directory for input to large language models, with flexible file selection.
...
For more information, visit: https://github.com/tmc/misc/ctx-plugins
EOF
}
