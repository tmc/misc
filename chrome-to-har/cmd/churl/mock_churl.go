...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(w, "churl - Mock Chrome-powered curl for JavaScript/SPA support\n\n")
		fmt.Fprintf(w, "Usage:\n")
	...
	// Run the mock churl command
	if err := runMock(ctx, url, opts); err != nil {
...
func runMock(ctx context.Context, url string, opts options) error {
	...
	if req.Header.Get("User-Agent") == "" {
		req.Header.Set("User-Agent", "churl/1.0 (mock mode)")
	}
	...
	case "text":
		// Very simple text extraction (this is a mock, so it's simplified)
		text := string(body)
