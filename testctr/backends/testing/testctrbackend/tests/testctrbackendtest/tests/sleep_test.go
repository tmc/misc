...
func TestContainerWithLongStartupAndWaitCondition(t *testing.T) {
	...
	c := testctr.New(t, "alpine:latest",
		testctr.WithCommand("sh", "-c", "sleep 2 && echo 'finally ready' && sleep infinity"),
	...
	// With the custom wait condition, this should take about 2 seconds
	if elapsed < 1*time.Second || elapsed > 3*time.Second {
		t.Errorf("Expected container to be ready after ~2s, but took %v", elapsed)
	}
...
func TestContainerWithLongInternalStartup_UsingWaitExec(t *testing.T) {
	...
	if elapsed < 3*time.Second || elapsed > 7*time.Second {
